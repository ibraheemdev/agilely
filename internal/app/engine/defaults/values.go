...
func NewHTTPBodyReader(readJSON, useUsernameNotEmail bool) *HTTPBodyReader {
	...
	return &HTTPBodyReader{
		...
		ReadJSON:    readJSON,
		Rulesets: map[string][]Rules{
			users.PageLogin:        {pidRules},
			users.PageRegister:     {pidRules, passwordRule},
			users.PageConfirm:      {Rules{FieldName: FormValueConfirm, Required: true}},
			users.PageRecoverStart: {pidRules},
			users.PageRecoverEnd:   {passwordRule},
		},
		Confirms: map[string][]string{
			users.PageRegister:   {FormValuePassword, engine.ConfirmPrefix + FormValuePassword},
			users.PageRecoverEnd: {FormValuePassword, engine.ConfirmPrefix + FormValuePassword},
		},
		Whitelist: map[string][]string{
			users.PageRegister: {FormValueEmail, FormValuePassword},
		},
	}
}

// Read the form pages
func (h HTTPBodyReader) Read(page string, r *http.Request) (engine.Validator, error) {
