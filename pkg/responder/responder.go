...
func (r *Responder) Respond(w http.ResponseWriter, req *http.Request, code int, page string, data engine.HTMLData) error {
	ctxData := req.Context().Value(engine.CTXKeyData)
	if ctxData != nil {
		if data == nil {
			data = engine.HTMLData{}
		}
		data.Merge(ctxData.(engine.HTMLData))
	}
...
func NewRedirector(renderer engine.Renderer, formValueName string) *Redirector {
...
func (r Redirector) redirectAPI(w http.ResponseWriter, req *http.Request, ro engine.RedirectOptions) error {
...
func (r Redirector) redirectNonAPI(w http.ResponseWriter, req *http.Request, ro engine.RedirectOptions) error {
	path := ro.RedirectPath
	redir := req.FormValue(r.FormValueName)
	if len(redir) != 0 && ro.FollowRedirParam {
		path = redir
	...
	if len(ro.Success) != 0 {
		engine.PutSession(w, engine.FlashSuccessKey, ro.Success)
	}
	if len(ro.Failure) != 0 {
		engine.PutSession(w, engine.FlashErrorKey, ro.Failure)
	}
