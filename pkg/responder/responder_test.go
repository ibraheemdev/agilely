...
type testRenderer struct {
	Callback func(context.Context, string, engine.HTMLData) ([]byte, string, error)
}
...
func (t testRenderer) Render(ctx context.Context, name string, data engine.HTMLData) ([]byte, string, error) {
	return t.Callback(ctx, name, data)
}

func testJSONRender(ctx context.Context, name string, data engine.HTMLData) ([]byte, string, error) {
...
func TestResponder(t *testing.T) {
	...
	r = r.WithContext(context.WithValue(context.Background(), engine.CTXKeyData, engine.HTMLData{
		"csrfname":  "csrf",
		"csrftoken": "12345",
	}))

	err := responder.Respond(w, r, http.StatusCreated, "some_template.tpl", engine.HTMLData{"auth_happy": true})
	if err != nil {
		t.Error(err)
	}
	...
	expectData := engine.HTMLData{
		"csrfname":   "csrf",
		"csrftoken":  "12345",
		"auth_happy": true,
	}

	var gotData engine.HTMLData
...
func TestResponseRedirectNonAPI(t *testing.T) {
	...
	renderer := testRenderer{
		Callback: func(ctx context.Context, name string, data engine.HTMLData) ([]byte, string, error) {
			return nil, "", nil
		},
	}
